...
class HartreeFock:
    def __init__(self, NobleGasModel, atomic_coordinates, useFockFast = False):
        ...
        self.useFockFast = useFockFast
        self.model_parameters = NobleGasModel.model_parameters
    ...
    def scf_cycle(self, NobleGasModel, max_scf_iterations = 200, mixing_fraction = 0.25, convergence_tolerance = 1e-7):
        ...
        for iteration in range(max_scf_iterations):
            ...
            if self.useFockFast:
                self.fock_matrix = fock_fast.calculate_fock_matrix_fast(self.hamiltonian_matrix, self.interaction_matrix, self.density_matrix, self.model_parameters, self.orbitals_per_atom)
            else:
